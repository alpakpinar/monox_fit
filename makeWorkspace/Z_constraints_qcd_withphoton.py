...
def cmodel(cid,nam,_f,_fOut, out_ws, diag, year,convention="BU"):
  ...
  if year==2017:
    CRs[2].add_nuisance('jesRelativeBal',-0.02)
    CRs[2].add_nuisance('jer_{YEAR}'.format(YEAR=year),-0.025)
  elif year==2018:
    CRs[2].add_nuisance('jesRelativeBal'.format(YEAR=year),-0.02)
  ...
  if year==2017:
    CRs[3].add_nuisance('jer_{YEAR}'.format(YEAR=year),0.04)
  elif year==2018:
    CRs[3].add_nuisance('jer_{YEAR}'.format(YEAR=year),0.015)
  else:
    raise RuntimeError("Year not recognized: " + str(year))
  # ############################ USER DEFINED ###########################################################
